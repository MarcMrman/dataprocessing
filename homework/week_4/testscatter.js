...
window.onload = function() {
	...
	function loadingPage(error, response) {
	  ...
	  // writing JSON
	  	...
	  	getGDP(response[0].responseText, response[1].responseText, response[2].responseText)

	};

	function getGDP(data, data1, data2){
		...
		// parsing api data
	  	...
	  	for (var j = 0; j < 10; j ++){
		  	...
		  	makeScatter(datasetHrs, hours_PY, gdp_PY, gdpJSON)
		//return [datasetWage, datasetHrs]
	}

	function makeScatter(data, hours, gdp, gdpJSON){
        ...
        // creating info window WERKT NOG NIET
        var tip = d3.tip()
      				.attr("class", "d3-tip")
        ...
        svg.selectAll("circle")
		   ...
		   .attr("cx", function(d, i){
		   		return scaleX(data[i][1]);
		   })
   			.attr("cy", function(d, i){
		   		return scaleY(data[i][0]);
		   	})
   			.attr("r", function(d, i){
   				return (data[i][0] / data[i][1]) / 2;
   			})
   			.attr("fill", function(d, i){
   				// GDP / hour worked ratio
   				if (data[i][0] / data[i][1] > data[0][0] / data[0][1]) {return "#c51b8a"}
   				else if (data[i][0] / data[i][1] < data[0][0] / data[0][1]) {return "#fa9fb5"}
   				else {return "#000000"};
   			})
